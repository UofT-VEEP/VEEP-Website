import React from 'react';
import NavBar from './Components/NavBar';
import 'bootstrap/dist/css/bootstrap.min.css';

import { Card, Accordion, Container, Row, Button, Toast } from 'react-bootstrap';
import './Styles/NavBar.css';
import './Styles/HomePage.css';

class HomePage extends React.Component {

  // Un-comment this for week 4 content 
  constructor(props){
     super(props);
     this.state = {
       showToast: false,
     };
     this.toastButton = this.toastButton.bind(this);
  }
  
  toastButton() {
    const press = !(this.state.showToast);
    this.setState({
      showToast: press,
    });
  }

  render() {
    return (
        
      <div>
        <NavBar/>
        <Container fluid>
          <Row>
          <Card id="Introduction">
          <Card.Img id="Uoft_logo" variant="top" src="https://upload.wikimedia.org/wikipedia/en/thumb/0/04/Utoronto_coa.svg/500px-Utoronto_coa.svg.png" alt='UofT'/>
            <Card.Body>
              <Card.Title id="Name">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Bella Huang</Card.Title>
              <Card.Text>
                <div>
                  <p>
                    &nbsp;&nbsp;&nbsp;&nbsp;ECE2T4<br />
                    &nbsp;&nbsp;&nbsp;&nbsp;Technical Member at VEEP<br />
                    &nbsp;&nbsp;&nbsp;&nbsp;Hi, my name is Bella and I love coding!<br />
                  </p>
                </div>
              </Card.Text>
               <Accordion defaultActiveKey="0">
                <Accordion.Item eventKey="0">
                  <Accordion.Header>View More</Accordion.Header>
                  <Accordion.Body>I also like reading and playing piano!</Accordion.Body>
                </Accordion.Item>
              </Accordion>
            </Card.Body>
          </Card>

          <Card id="New">
            <Card.Img id="Uoft_logo2" variant="top" src="data:image/png;base64," alt='VEEP'/>
              <Card.Body>
                <Card.Text>
                  <div>
                    <p>
                      &nbsp;&nbsp;&nbsp;&nbsp;<b>VEEP</b><br />
                      &nbsp;&nbsp;&nbsp;&nbsp;Student Club<br />
                      &nbsp;&nbsp;&nbsp;&nbsp;VEEP is Volunteer Engineering Experience Program<br />
                    </p>
                  </div>
                </Card.Text>
                  <Button variant="primary" onClick={this.toastButton} className="mb-2">
                    Click Me
                  </Button>
                  <Toast show={this.state.showToast} onClose={this.toastButton}>
                    <Toast.Header>
                      <img
                        src="https://upload.wikimedia.org/wikipedia/en/thumb/0/04/Utoronto_coa.svg/500px-Utoronto_coa.svg.png"
                        className="toast_img"
                        alt="UofT"
                        width="30"
                        height="25"
                      />
                      <strong className="me-auto">Bella Huang</strong>
                      <small>just now</small>
                    </Toast.Header>
                    <Toast.Body>Woohoo, you're reading this text in a Toast!</Toast.Body>
                  </Toast>
              </Card.Body>
          </Card>
          </Row>
          </Container>
        
      </div>
    )
  }
}

export default HomePage;